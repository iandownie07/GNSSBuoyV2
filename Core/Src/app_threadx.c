...
UINT App_ThreadX_Init(VOID *memory_ptr)
{
  ...
  TX_BYTE_POOL *byte_pool = (TX_BYTE_POOL*)memory_ptr;
   ...
   /* USER CODE BEGIN App_ThreadX_MEM_POOL */
	...
	ret = tx_byte_allocate(byte_pool, (VOID**) &pointer, THREAD_EXTRA_EXTRA_LARGE_STACK_SIZE, TX_NO_WAIT);
	if (ret != TX_SUCCESS){
	  return ret;
	}
	...
	ret = tx_thread_create(&waves_thread, "waves thread", waves_thread_entry, 0, pointer,
		THREAD_EXTRA_EXTRA_LARGE_STACK_SIZE, MID, MID, TX_NO_TIME_SLICE, TX_DONT_START);
	if (ret != TX_SUCCESS){
	  return ret;
	}
	#endif
	...
	/* Create the main thread.  */
  	ret = tx_thread_create(&sd_thread, "sd thread", sd_thread_entry, 0, pointer, 
                         THREAD_EXTRA_LARGE_STACK_SIZE, MID, MID, TX_NO_TIME_SLICE, TX_DONT_START);
...
void startup_thread_entry(ULONG thread_input){
	...
	if (tx_return == TX_SUCCESS) {
		...
		tx_return = tx_thread_reset(&sd_thread);
		if (tx_return == TX_NOT_DONE){
			tx_thread_terminate(&sd_thread);
			tx_thread_reset(&sd_thread);
		}
...
void gnss_thread_entry(ULONG thread_input){
	...
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_7, GPIO_PIN_SET); // LD1
...
void waves_thread_entry(ULONG thread_input){
	...
	real16_T E[TOTAL_BANDS_PER_SPECTRUM];
	real16_T EW[TOTAL_BANDS_PER_SPECTRUM];
	real16_T Dp;
	real16_T Hs;
	real16_T Tp;
	real16_T b_fmax;
	real16_T b_fmin;
	signed char a1[TOTAL_BANDS_PER_SPECTRUM];
	signed char a2[TOTAL_BANDS_PER_SPECTRUM];
	signed char b1[TOTAL_BANDS_PER_SPECTRUM];
	signed char b2[TOTAL_BANDS_PER_SPECTRUM];
	unsigned char check[TOTAL_BANDS_PER_SPECTRUM];
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, GPIO_PIN_SET); // LD2

	/* Call the entry-point 'NEDwaves_memlight'. */
	NEDwaves_memlight(north, east, down, gnss->sample_window_freq, &Hs, &Tp, &Dp, E, EW,
	...
	memcpy(&(sbd_message.EW_array[0]), &(EW[0]), TOTAL_BANDS_PER_SPECTRUM * sizeof(real16_T));
	memcpy(&(sbd_message.E_array[0]), &(E[0]), TOTAL_BANDS_PER_SPECTRUM * sizeof(real16_T));
	//memcpy(&sbd_message.f_min, &b_fmin, sizeof(real16_T));
	//memcpy(&sbd_message.f_max, &b_fmax, sizeof(real16_T));
	memcpy(&(sbd_message.a1_array[0]), &(a1[0]), TOTAL_BANDS_PER_SPECTRUM * sizeof(signed char));
	memcpy(&(sbd_message.b1_array[0]), &(b1[0]), TOTAL_BANDS_PER_SPECTRUM * sizeof(signed char));
	memcpy(&(sbd_message.a2_array[0]), &(a2[0]), TOTAL_BANDS_PER_SPECTRUM * sizeof(signed char));
	memcpy(&(sbd_message.b2_array[0]), &(b2[0]), TOTAL_BANDS_PER_SPECTRUM * sizeof(signed char));
	memcpy(&(sbd_message.cf_array[0]), &(check[0]), TOTAL_BANDS_PER_SPECTRUM * sizeof(unsigned char));

	/*int size = TOTAL_BANDS_PER_SPECTRUM;
	printf("Array a1:\n");
	...
	*/
	...
	if (tx_thread_resume(&sd_thread) != TX_SUCCESS){
		shut_it_all_down();
		HAL_NVIC_SystemReset();
	}
	...
	// THIS THREAD STARTER HAS BEEN COPIED FROM THE IRIDIUM THREAD 14/10/24
...
void sd_thread_entry(ULONG thread_input){
	...
	float sbd_timestamp = sd->get_timestamp(sd);
	memcpy(&sbd_message.timestamp, &sbd_timestamp, sizeof(float));

	sd->save_message(sd);
	HAL_Delay(10000);
	// THIS THREAD STARTER HAS BEEN COPIED FROM THE IRIDIUM THREAD 9/02/25
	if (tx_thread_resume(&end_of_cycle_thread) != TX_SUCCESS){
		shut_it_all_down();
		HAL_NVIC_SystemReset();
	}
...
void end_of_cycle_thread_entry(ULONG thread_input){
	...
	ULONG error_occured_flags = GNSS_ERROR | MEMORY_ALLOC_ERROR |
...
void shut_it_all_down(void)
{
// WE MAY HAVE TO DISACTIVATE A LITTLE HERE
