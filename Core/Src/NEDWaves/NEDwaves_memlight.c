...
#define SECTIONS 4  // Number of SOS sections

// SOS coefficients: {b0, b1, b2, a0, a1, a2}
double sos[SECTIONS][6] = {
  // STRAIGHT FROM CHEBY2 
    { 0.9504896627934517, -0.9504896627934517,  0.0,  1.0, -0.9776524360590855,  0.0 },
    { 1.0, -1.999860854341652,  1.0000000000000002,  1.0, -1.9594582239065221,  0.9600950677098707 },
    { 1.0, -1.9995482321975406,  1.0000000000000002,  1.0, -1.9712673467814488,  0.9722163293785115 },
    { 1.0, -1.9992975640090709,  0.9999999999999998,  1.0, -1.988788472600348,  0.9899953606269386 }
};

// Separate state history: 2 past inputs, 2 past outputs per section
double state[SECTIONS][4] = {0};

double sos_filter(double input) {
  double xn = input;  // Current input sample

  for (int i = 0; i < SECTIONS; i++) {
      // Compute new output using correct past values
      double yn = sos[i][0] * xn      // b0 * x[n]
                + sos[i][1] * state[i][0]  // b1 * x[n-1]
                + sos[i][2] * state[i][1]  // b2 * x[n-2]
                - sos[i][4] * state[i][2]  // a1 * y[n-1]
                - sos[i][5] * state[i][3]; // a2 * y[n-2]

      // Shift past values correctly
      state[i][1] = state[i][0];  // x[n-2] = x[n-1]
      state[i][0] = xn;           // x[n-1] = x[n]

      state[i][3] = state[i][2];  // y[n-2] = y[n-1]
      state[i][2] = yn;           // y[n-1] = y[n]

      xn = yn;  // Pass output to next section
  }

  return xn;
}

// Function to reset filter history before processing a new time-series
void reset_filter() {
  for (int i = 0; i < SECTIONS; i++) {
      for (int j = 0; j < 4; j++) {
          state[i][j] = 0.0;
...
void NEDwaves_memlight(emxArray_real32_T *north, emxArray_real32_T *east,
                       ...
                       real16_T *Tp, real16_T *Dp, real16_T E[42], real16_T EW[42],
                       ...
                       signed char b2[42], unsigned char check[42])
{
  ...
  fs = 5;
  double orig_wpts = rt_roundd_snf(256.0*fs);
  int exp = (int) floor(log2(orig_wpts));
  wpts = pow(2.0, exp);
  printf("wpts: %d", wpts);
  ...
  windows = floor(2.666667 * ((double)east->size[1] / wpts - 1.0) + 1.0); 
  ...
  fpindex = f->size[1];
  for (i = 0; i < fpindex; i++) {
    bad_data[i] = (f_data[i] > MAXF); 
    //printf("f data %lf\n", f_data[i]);
    //printf("rawf data %lf\n", rawf_data[i]);
  }
  nullAssignment(f, bad);
  f_data = f->data;
  //printf("Size of f data after bad %d\n", (int)f->size[1]);
  ...
  fe = b_combineVectorElements(filtereddata) / (float)r5->size[1]; //find mean and replace outliers
  fpindex = r4->size[1] - 1;
  emxFree_int32_T(&r5);
  for (i = 0; i <= fpindex; i++) {
  ...
  if ((int)windows - 1 >= 0) {
    ...
    y = d1 * fs * 0.375;
    b_end = rawf->size[1] - 1;
    y_tmp = rt_roundd_snf(wpts / 2.0) * fs * 0.375;
    c_end = rawf->size[1] - 1;
    d_end = rawf->size[1] - 1;
    e_end = rawf->size[1] - 1;
    f_end = rawf->size[1] - 1;
  ...
  for (q = 0; q < i; q++) {
    Nyquist = (((double)q + 1.0) - 1.0) * floor(0.375 * wpts);  // CHANGED TO 1-0.625
    u->size[0] = 1;
    ...
    float sum_before = 0;
    float sum_after = 0;
    for (fpindex = 0; fpindex <= i3; fpindex++) { // ##2 filtering
      filtereddata_data[fpindex] = sos_filter(u_data[fpindex]); //u_data[fpindex]; 
      //filtereddata_data[fpindex + 1] = 
      //    (float)alpha * filtereddata_data[fpindex] +
      //    (float)alpha * (u_data[fpindex + 1] - u_data[fpindex]);
          sum_before += fabsf(u_data[fpindex]);
          sum_after += fabsf(filtereddata_data[fpindex]);
    }
    reset_filter();
    printf("sum_before %f ", sum_before);
    printf("sum_after: %f\n", sum_after);
    i3 = u->size[0] * u->size[1];
    u->size[0] = 1;
    ...
    for (i3 = 0; i3 < fpindex; i3++) {
      u_data[i3] = filtereddata_data[i3]; // Here they are transferred back into u_data
    }
    i3 = filtereddata->size[0] * filtereddata->size[1];
    filtereddata->size[0] = 1;
    ...
    sum_before = 0;
    sum_after = 0;
    for (fpindex = 0; fpindex <= i3; fpindex++) {
        filtereddata_data[fpindex] = sos_filter(v_data[fpindex]); //v_data[fpindex]; 
        //filtereddata_data[fpindex + 1] =
        //  (float)alpha * filtereddata_data[fpindex] +
        //  (float)alpha * (v_data[fpindex + 1] - v_data[fpindex]);
          sum_before += fabsf(v_data[fpindex]);
          sum_after += fabsf(filtereddata_data[fpindex]);
    }
    reset_filter();
    printf("sum_before %f ", sum_before);
    ...
    sum_after = 0;
    for (fpindex = 0; fpindex <= i3; fpindex++) {
       filtereddata_data[fpindex] = sos_filter(w_data[fpindex]); //w_data[fpindex];
       //filtereddata_data[fpindex + 1] =
       //   (float)alpha * filtereddata_data[fpindex] +
       //   (float)alpha * (w_data[fpindex + 1] - w_data[fpindex]);
          sum_before += fabsf(w_data[fpindex]);
          sum_after += fabsf(filtereddata_data[fpindex]);
    }
    reset_filter();
    printf("sum_before %f ", sum_before);
    ...
    nx = taper->size[1];
    for (fpindex = 0; fpindex < nx; fpindex++) { // ##3 window
      taper_data[fpindex] = 0.5*(1 - cos(2*taper_data[fpindex]));
    ...
    /*  then rescale to regain the same original variance */ // ##4 variance compensation
    ...
    /*  throw out the mean (first coef), shift the array left and add a zero (to make it the right
     * length) */
    ...
    for (b_i = 0; b_i <= end; b_i++) {
      if (rawf_data[b_i] < MAXF) {
        nx++;
      }
    }
    i3 = r8->size[0] * r8->size[1];
    r8->size[0] = 1;
    r8->size[1] = nx; // i3 = r8->size[0] * r8->size[1];
    ...
    fpindex = r8->size[1];
    for (i3 = 0; i3 < fpindex; i3++) { 
      u_re_tmp = b_u_data[r1[i3] - 1].re; // r1[0] = 1, etc. so we get the right index
    ...
    i3 = UVwindow->size[0] * UVwindow->size[1]; // cospectra
    UVwindow->size[0] = 1;
    ...
    for (mi = 0; mi < i1; mi++) {
      ...
      UU[b_i] += b_combineVectorElements(c_u) / (float)fpindex; // works correctly to sum the windows & merge bins
      if ((Nyquist - 3.0) + 1.0 > Nyquist) {
        i3 = -1;
        i5 = -1;
      } else {
   ...
   */
  for (fpindex = 0; fpindex < 42; fpindex++) { // ##5 ENBW
    float ENBW = 1;
    //float ENBW = 1.5; // all the windows should be multiplied by the ENBW
    w_re = UU[fpindex] / (float)(windows*ENBW); // divide the sum by the number of windows to get the average
    UU[fpindex] = w_re;
    w_im = VV[fpindex] / (float)(windows*ENBW); 
    VV[fpindex] = w_im;
    v_re = WW[fpindex] / (float)(windows*ENBW);
    WW[fpindex] = v_re;
    fe = UV[fpindex].re;
    u_re_tmp = UV[fpindex].im;
    if (u_re_tmp == 0.0F) {
      x = fe / (float)(windows*ENBW);
      u_re = 0.0F;
    } else if (fe == 0.0F) {
      x = 0.0F;
      u_re = u_re_tmp / (float)(windows*ENBW);
    } else {
      x = fe / (float)(windows*ENBW);// I have forgotten to divide by ENBW here
      u_re = u_re_tmp / (float)(windows*ENBW);
    }
    UV[fpindex].re = x;
    UV[fpindex].im = u_re;
    ...
    if (u_re_tmp == 0.0F) {
      ...
      x = 0.0F;
      fe = u_re_tmp / (float)(windows*ENBW);
    } else {
      x = fe / (float)(windows*ENBW);
      fe = u_re_tmp / (float)(windows*ENBW);
    }
  ...
  for (i = 0; i < fpindex; i++) {
    ...
    rawf_data[i] = Nyquist * Nyquist; // put the value from (f_data (larger step) * 2pi)^2 into rawf_data (smaller step)
  }
  if (rawf->size[1] == 42) {
    for (i = 0; i < 42; i++) {
      b_E[i] = y_tmp_tmp[i] / (float)rawf_data[i]; 
      b_EW[i] = WW[i] / (float)rawf_data[i];
      //printf("b_E %f\n", b_E[i]);
      //printf("b_EW %f\n", b_EW[i]);
      printf("RAWF %f\n", (float)rawf_data[i]);
    }
  ...
  *b_fmax = doubleToHalf(b_maximum(f));
  ...
  for (i = 0; i < 42; i++) {
    ...
    a2[i] = i4;
    w_im = roundf(2.0F * UV[i].re / w_re * 100.0F); // b2 is calculated here
    if (w_im < 128.0F) {
      if (w_im >= -128.0F) {
        i4 = (signed char)w_im;
      } else {
