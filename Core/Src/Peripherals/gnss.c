...
void gnss_process_message(GNSS* self)
{
				 ...
				 &message_class, &message_id, (char*)payload, sizeof(payload), &buf_end))
	{
		...
		int8_t flags = payload[UBX_NAV_PVT_VALID_FLAGS1_INDEX];
		numSV = payload[UBX_NAV_PVT_NUMSV_INDEX];
		//printf("%ld\n", self->messages_processed);
		//printf("PDOP %d\n", pDOP);
		//printf("numSV %d\n", numSV);
		//printf("sAcc %d\n", sAcc);
		...
		size_t message_size)
{
    ...
    while (fail_counter++ < 10) {
		...
		HAL_UART_Receive_DMA(self->gnss_uart_handle, &(msg_buf[0]),
			sizeof(msg_buf));
		printf("IT is: %s\n", msg_buf); // this returns \mu when working
		__HAL_DMA_DISABLE_IT(self->gnss_dma_handle, DMA_IT_HT);
		if (tx_event_flags_get(self->control_flags, GNSS_CONFIG_RECVD, TX_OR_CLEAR,
 ...
 */
	static uint32_t get_timestamp(GNSS* self)
	{
		...
		if (HAL_RTC_GetTime(self->rtc_handle, &rtc_time, RTC_FORMAT_BIN) != HAL_OK) {
   		 // Error handling, maybe log an error
